...
SELECT P.Pname
FROM PLAYER P
WHERE P.Draft_year > 2005 AND P.Player_ID IN(SELECT TPA.Player_ID
                                             FROM TEAM_PLAYER_ARENA TPA
                                             WHERE TPA.Team_ID IN (SELECT T.Team_ID
                                                                   FROM TEAM T
                                                                   WHERE T.Tname = 'Hornets'
                                                                    )
                                            );
                                            
-- 2. Print the cities where the games played had scores between 12 and 20
SELECT DISTINCT A.city
FROM ARENA A
WHERE A.ARENA_ID IN(SELECT TPA.ARENA_ID
                    FROM TEAM_PLAYER_ARENA TPA
                    WHERE TPA.Points BETWEEN 12 AND 20
                   );
                   
-- 3. Print the names of players that belong to team that comes from a city that begins with 'Ch' and were drafted in 2007
SELECT P.Pname
FROM PLAYER P
WHERE P.Draft_year = 2007 AND P.Player_ID IN(SELECT TPA.Player_ID
                                             FROM TEAM_PLAYER_ARENA TPA
                                             WHERE TPA.Team_ID IN (SELECT T.Team_ID
                                                                   FROM TEAM T
                                                                   WHERE T.city LIKE 'Ch%'
                                                                   )
                                            );
                                            
-- 4. Print the names of the teams that played a home game
SELECT T.Tname
FROM TEAM T
WHERE T.Team_ID IN (SELECT TPA.Team_ID
                    FROM TEAM_PLAYER_ARENA TPA
                    WHERE TPA.ARENA_ID IN (SELECT A.ARENA_ID
                                           FROM ARENA A
                                           WHERE T.city = A.city
                                           )
                   );
                   
-- 5. Print the points and name of the player with the highest score
SELECT TPA.Points AS HighestScore, P.Pname
FROM TEAM_PLAYER_ARENA TPA, PLAYER P
WHERE TPA.Player_ID = P.Player_ID AND TPA.Points =(SELECT MAX(TPA.Points)
                                                   FROM TEAM_PLAYER_ARENA TPA
                                                  );

-- 6. Print the names of the teams that played in all arenas ???????
SELECT T.Tname
FROM TEAM T
INNER JOIN 
TEAM_PLAYER_ARENA TPA ON TPA.Team_ID = T.Team_ID
GROUP BY T.Tname 
HAVING COUNT(DISTINCT TPA.ARENA_ID) = (SELECT COUNT(DISTINCT A.ARENA_ID) FROM ARENA A);
-----------------------------------------------------------------------------------------------------
COMMIT;		
